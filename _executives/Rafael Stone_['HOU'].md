---
bpm_rank: 114
draft_bust_list: null
draft_description: Rafael Stone ranks in the 9th percentile for drafting among executives
  in our database. Stone's draft picks average out to be worth the equivalent of 14
  spots lower than they were picked due to their terrible drafting.
draft_num_players: 9
draft_rating: -13.8
draft_rating_grade: F
draft_win_list: null
end_year: 2026
final_bpm: -52647
href: stonera99x
is_active: true
name: Rafael Stone
signing_description: 'Rafael Stone ranks in the 79th percentile for free agency signings
  among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Daishen Nix in 2021.</li>

  <li>Signed Jae''Sean Tate in 2020.</li>

  <li>Signed Kenyon Martin Jr. in 2020.</li>

  </ul>'
signing_rating: -0.31
signing_rating_grade: B+
signing_win_list: '<ul>

  <li>Signed Alperen Şengün in 2021.</li>

  <li>Signed Fred VanVleet in 2023.</li>

  <li>Signed Boban Marjanović in 2023.</li>

  </ul>'
start_year: 2020
team:
- Houston Rockets
team_shortcode:
- HOU
trade_description: 'Rafael Stone ranks in the 25th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Rodions Kurucs, Tari Eason, Reed Sheppard, Dante Exum, MarJon Beauchamp,
  Victor Oladipo to HOU.</li>

  <li>Lost Jalen Pickett to IND.</li>

  <li>Lost James Harden to BRK.</li>

  </ul>'
trade_rating: -0.12
trade_rating_grade: D-
trade_win_list: '<ul>

  <li>Added Christian Wood, Luka Garza to HOU.</li>

  <li>Lost Trevor Ariza, Isaiah Stewart to DET.</li>

  </ul>'
---