---
badges: ''
bpm_rank: 51
draft_bust_list: '<ul>

  <li>Drafted Harrison Barnes with 7th pick but was worth 54th pick.</li>

  <li>Drafted James Wiseman with 2th pick but was worth 46th pick.</li>

  <li>Drafted Jordan Poole with 28th pick but was worth 52th pick.</li>

  </ul>'
draft_description: Bob Myers ranks in the 40th percentile for drafting among executives
  in our database. Myers's draft picks average out to be worth the equivalent of 4
  spots lower than they were picked due to their terrible drafting.
draft_num_players: 14
draft_rating: -3.7
draft_rating_grade: C-
draft_win_list: '<ul>

  <li>Drafted Draymond Green with 35th pick but was worth 3th pick.</li>

  <li>Drafted Kevon Looney with 30th pick but was worth 11th pick.</li>

  <li>Drafted Nico Mannion with 48th pick but was worth 25th pick.</li>

  </ul>'
end_year: 2023
final_bpm: 14321
href: myersbo99x
is_active: false
name: Bob Myers
signing_description: 'Bob Myers ranks in the 85th percentile for free agency signings
  among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Marreese Speights in 2013.</li>

  <li>Signed Patrick McCaw in 2016.</li>

  <li>Signed Damion Lee in 2019.</li>

  </ul>'
signing_rating: -0.19
signing_rating_grade: A-
signing_win_list: '<ul>

  <li>Signed Kevin Durant in 2016.</li>

  <li>Signed Kevin Durant in 2018.</li>

  <li>Signed Draymond Green in 2019.</li>

  </ul>'
start_year: 2012
team:
- Golden State Warriors
team_shortcode:
- GSW
trade_description: 'Bob Myers ranks in the 70th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Treveon Graham, Shabazz Napier, D''Angelo Russell to GSW.</li>

  <li>Lost Kevin Durant to BRK.</li>

  </ul>'
trade_rating: 0.13
trade_rating_grade: B
trade_win_list: '<ul>

  <li>Added Andre Iguodala, Kevin Murphy to GSW.</li>

  <li>Lost Andris Biedriņš, Richard Jefferson, Brandon Rush, Rodney Hood, Tyrone Wallace,
  Josh Hart, Alpha Kaba to UTA.</li>

  <li>Lost Thomas Welsh to DEN.</li>

  </ul>'
---