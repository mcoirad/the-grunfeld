---
bpm_rank: 20
draft_bust_list: '<ul>

  <li>Drafted Dennis Smith Jr. with 9th pick but was worth 50th pick.</li>

  <li>Drafted Josh Green with 18th pick but was worth 44th pick.</li>

  </ul>'
draft_description: Donn Nelson ranks in the 59th percentile for drafting among executives
  in our database. Nelson's draft picks average out to be worth the equivalent of
  1 spot higher than they were picked due to his good drafting.
draft_num_players: 8
draft_rating: 1.2
draft_rating_grade: C+
draft_win_list: '<ul>

  <li>Drafted A.J. Hammons with 46th pick but was worth 23th pick.</li>

  </ul>'
end_year: 2021
final_bpm: 119850
href: nelsodo99x
name: Donn Nelson
signing_description: 'Donn Nelson ranks in the 60th percentile for free agency signings
  among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Dorian Finney-Smith in 2016.</li>

  <li>Signed Wesley Matthews in 2015.</li>

  <li>Signed J.J. Barea in 2006.</li>

  </ul>'
signing_rating: -0.38
signing_rating_grade: B-
signing_win_list: '<ul>

  <li>Signed Luka Dončić in 2018.</li>

  <li>Signed Kristaps Porziņģis in 2019.</li>

  <li>Signed Vince Carter in 2011.</li>

  </ul>'
start_year: 2005
team:
- Dallas Mavericks
team_shortcode:
- DAL
trade_description: 'Donn Nelson ranks in the 87th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Luka Dončić to DAL.</li>

  <li>Lost Trae Young, Cam Reddish to ATL.</li>

  </ul>'
trade_rating: 0.28
trade_rating_grade: A
trade_win_list: '<ul>

  <li>Added Malik Allen, Jason Kidd, Antoine Wright to DAL.</li>

  <li>Lost Maurice Ager, DeSagana Diop, Devin Harris, Trenton Hassell, Keith Van Horn,
  Ryan Anderson, Jordan Crawford to NJN.</li>

  </ul>'
---