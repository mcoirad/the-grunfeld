---
bpm_rank: 112
draft_bust_list: '<ul>

  <li>Drafted John Wall with 1th pick but was worth 2th pick.</li>

  <li>Drafted Jan Veselý with 6th pick but was worth 43th pick.</li>

  <li>Drafted Nick Young with 16th pick but was worth 40th pick.</li>

  </ul>'
draft_description: Ernie Grunfeld ranks in the 44th percentile for drafting among
  executives in our database. That's basically average. It could worse though lol.
draft_num_players: 28
draft_rating_bpm: -3.2
draft_rating_grade_bpm: D+
draft_rating_grade_vorp: C-
draft_rating_percentile_bpm: 39.44954128440367
draft_rating_percentile_vorp: 44.95412844036697
draft_rating_vorp: -0.5
draft_total_bpm: 95751
draft_total_percentile_bpm: 93.90243902439025
draft_total_percentile_vorp: 93.90243902439025
draft_total_vorp: 102
draft_win_list: '<ul>

  <li>Drafted Michael Redd with 43th pick but was worth 1th pick.</li>

  <li>Drafted Charlie Ward with 26th pick but was worth 6th pick.</li>

  <li>Drafted Dominic McGuire with 47th pick but was worth 30th pick.</li>

  </ul>'
end_year: 2019
final_bpm: -49348
href: grunfer01x
is_active: false
name: Ernie Grunfeld
signing_description: 'Ernie Grunfeld ranks in the 56th percentile for free agency
  signings among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Kelly Oubre Jr. in 2015.</li>

  <li>Signed DeShawn Stevenson in 2006.</li>

  <li>Signed Ian Mahinmi in 2016.</li>

  </ul>'
signing_rating_bpm: -0.45
signing_rating_grade_vorp: C+
signing_rating_vorp: -0.02
signing_win_list: '<ul>

  <li>Signed Gilbert Arenas in 2003.</li>

  <li>Signed Anthony Mason in 1991.</li>

  <li>Signed Otto Porter in 2017.</li>

  </ul>'
start_year: 2003
team:
- New York Knicks
- Milwaukee Bucks
- Washington Wizards
team_shortcode:
- NYK
- MIL
- WAS
trade_description: 'Ernie Grunfeld ranks in the 57th percentile for trading among
  executives in our database. '
trade_lose_list: '<ul>

  <li>Added Desmond Mason, Gary Payton to MIL.</li>

  <li>Lost Ray Allen, Ronald Murray, Kevin Ollie, Luke Ridnour to SEA.</li>

  </ul>'
trade_rating_bpm: -0.08
trade_rating_grade_vorp: C+
trade_rating_vorp: 0.01
trade_win_list: '<ul>

  <li>Added Marcus Camby to NYK.</li>

  <li>Lost Sean Marks, Charles Oakley to TOR.</li>

  </ul>'
---