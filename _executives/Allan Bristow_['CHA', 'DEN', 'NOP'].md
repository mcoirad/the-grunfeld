---
bpm_rank: 148
draft_bust_list: null
draft_description: This executive has only drafted 1 player and thus doesn't have
  a draft grade.
draft_num_players: 1
draft_rating: null
draft_rating_grade: null
draft_win_list: '<ul>

  </ul>'
end_year: 2005
final_bpm: -133781
href: bristal01x
name: Allan Bristow
signing_description: 'Allan Bristow ranks in the 99th percentile for free agency signings
  among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Dean Garrett in 1997.</li>

  <li>Signed Kiwane Garris in 1997.</li>

  </ul>'
signing_rating: 0.0
signing_rating_grade: S
signing_win_list: '<ul>

  <li>Signed Jimmy King in 1997.</li>

  <li>Signed Harold Ellis in 1997.</li>

  </ul>'
start_year: 2004
team:
- Charlotte Hornets
- Denver Nuggets
- New Orleans Pelicans
team_shortcode:
- CHA
- DEN
- NOP
trade_description: 'Allan Bristow ranks in the 11th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Bobby Jackson to DEN.</li>

  <li>Lost James Cotton, Rashard Lewis to SEA.</li>

  </ul>'
trade_rating: -0.25
trade_rating_grade: F
trade_win_list: '<ul>

  <li>Added Priest Lauderdale to DEN.</li>

  <li>Lost Efthimis Rentzias, Hanno Möttölä to ATL.</li>

  </ul>'
---