---
bpm_rank: 132
draft_bust_list: null
draft_description: Billy McKinney ranks in the 24th percentile for drafting among
  executives in our database. McKinney's draft picks average out to be worth the equivalent
  of 7 spots lower than they were picked due to their terrible drafting.
draft_num_players: 8
draft_rating: -7.2
draft_rating_grade: D-
draft_win_list: null
end_year: 1995
final_bpm: -87831
href: mckinbi01x
is_active: false
name: Billy McKinney
signing_description: 'Billy McKinney ranks in the 11th percentile for free agency
  signings among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Sam Mitchell in 1989.</li>

  <li>Signed Rafael Addison in 1994.</li>

  <li>Signed Tod Murphy in 1989.</li>

  </ul>'
signing_rating: -0.63
signing_rating_grade: F
signing_win_list: '<ul>

  <li>Signed Tracy Moore in 1993.</li>

  <li>Signed Oliver Miller in 1994.</li>

  </ul>'
start_year: 1992
team:
- Minnesota Timberwolves
- Detroit Pistons
team_shortcode:
- MIN
- DET
trade_description: 'Billy McKinney ranks in the 7th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Randy Breuer, Chris Smith to MIN.</li>

  <li>Lost Brad Lohaus, Duane Cooper to MIL.</li>

  </ul>'
trade_rating: -0.38
trade_rating_grade: F
trade_win_list: '<ul>

  <li>Added Pete Chilcutt, Jevon Crudup, Cal Bowdler to DET.</li>

  <li>Lost Olden Polynice to SAC.</li>

  </ul>'
---