---
bpm_rank: 68
draft_bust_list: null
draft_description: This executive has only drafted 1 player and thus doesn't have
  a draft grade.
draft_num_players: 1
draft_overall_percentile_vorp: null
draft_rating_bpm: null
draft_rating_grade_bpm: null
draft_rating_grade_vorp: null
draft_rating_percentile_bpm: null
draft_rating_percentile_vorp: null
draft_rating_vorp: null
draft_total_bpm: 37658
draft_total_grade_bpm: B-
draft_total_grade_vorp: B-
draft_total_percentile_bpm: 62
draft_total_percentile_vorp: 62
draft_total_vorp: 18
draft_win_list: '<ul>

  </ul>'
end_year: 1999
final_bpm: -4013
final_vorp: 9
href: calipjo99x
is_active: false
name: John Calipari
signing_description: 'John Calipari ranks in the 9th percentile for free agency signings
  among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Tony Massenburg in 1996.</li>

  <li>Signed Xavier McDaniel in 1996.</li>

  <li>Signed Sherman Douglas in 1997.</li>

  </ul>'
signing_num_players: 25
signing_overall_percentile_bpm: 32
signing_overall_percentile_vorp: 10
signing_rating_bpm: -1
signing_rating_grade_bpm: D-
signing_rating_grade_vorp: F
signing_rating_percentile_bpm: 24
signing_rating_percentile_vorp: 10
signing_rating_vorp: 0
signing_total_bpm: -28394
signing_total_grade_bpm: D+
signing_total_grade_vorp: F
signing_total_percentile_bpm: 39
signing_total_percentile_vorp: 10
signing_total_vorp: -1
signing_win_list: '<ul>

  <li>Signed Scott Burrell in 1999.</li>

  <li>Signed Damon Jones in 1999.</li>

  </ul>'
start_year: 1996
team:
- Brooklyn Nets
team_shortcode:
- BRK
trade_description: 'John Calipari ranks in the 24th percentile for trading among executives
  in our database. '
trade_lose_list: '<ul>

  <li>Added Elliot Perry, Chris Carr, Bill Curley, Stephon Marbury to NJN.</li>

  <li>Lost Brian Evans, Wally Szczerbiak to MIN.</li>

  <li>Lost Sam Cassell, Chris Gatling to MIL.</li>

  </ul>'
trade_num: 41
trade_overall_percentile_bpm: 42
trade_overall_percentile_vorp: 31
trade_rating_bpm: 0
trade_rating_grade_bpm: C
trade_rating_grade_vorp: D-
trade_rating_percentile_bpm: 51
trade_rating_percentile_vorp: 24
trade_rating_vorp: 0
trade_total_bpm: -13277
trade_total_grade_bpm: D
trade_total_grade_vorp: D+
trade_total_percentile_bpm: 33
trade_total_percentile_vorp: 38
trade_total_vorp: -8
trade_win_list: '<ul>

  <li>Added Sam Cassell, Chris Gatling, Jim Jackson, George McCloud, Eric Montross
  to BRK.</li>

  <li>Lost Shawn Bradley, Ed O''Bannon, Robert Pack, Khalid Reeves to DAL.</li>

  </ul>'
---