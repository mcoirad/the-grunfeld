---
bpm_rank: 141
draft_bust_list: null
draft_description: Travis Schlenk ranks in the 54th percentile for drafting among
  executives in our database. That's basically average. It could worse though lol.
draft_num_players: 10
draft_rating: -0.2
draft_rating_grade: C+
draft_win_list: null
end_year: 2022
final_bpm: -107942
href: schletr01x
is_active: false
name: Travis Schlenk
signing_description: 'Travis Schlenk ranks in the 58th percentile for free agency
  signings among executives in our database. '
signing_lose_list: '<ul>

  <li>Signed Bruno Fernando in 2019.</li>

  <li>Signed Isaiah Taylor in 2017.</li>

  <li>Signed Solomon Hill in 2020.</li>

  </ul>'
signing_rating: -0.41
signing_rating_grade: C+
signing_win_list: '<ul>

  <li>Signed Trae Young in 2018.</li>

  <li>Signed Bogdan Bogdanovic in 2020.</li>

  <li>Signed Dewayne Dedmon in 2017.</li>

  </ul>'
start_year: 2017
team:
- Atlanta Hawks
team_shortcode:
- ATL
trade_description: 'Travis Schlenk ranks in the 75th percentile for trading among
  executives in our database. '
trade_lose_list: '<ul>

  <li>Added Trae Young, Cam Reddish to ATL.</li>

  <li>Lost Luka Dončić to DAL.</li>

  </ul>'
trade_rating: 0.17
trade_rating_grade: B+
trade_win_list: '<ul>

  <li>Added Clint Capela, Nene to ATL.</li>

  <li>Lost Evan Turner, Aleksej Pokusevski to MIN.</li>

  <li>Lost Pelle Larsson to HOU.</li>

  </ul>'
---